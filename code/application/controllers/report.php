...
class Report extends CI_Controller {
	...
	public function index() {
		...
		$contact1 = $contact2 = 0;
		//xiejy  修改 客户欠款、供应商欠款 非管理员显示0问题 改成调用 get_contact_purview2()
		$list = $this->data_model->get_contact($this->common_model->get_contact_purview2(),'a.isDelete=0');
		foreach ($list as $arr=>$row) {			    
		    if ($row['type']==-10) {
			    $contact1 += $row['amount']; 
			} elseif ($row['type']==10) {
	...
	public function salesDetail_detail() {
        ...
        $sum8 = $sum9 = $sum10 = $sum11 = 0;
		foreach ($list as $arr=>$row) {
//		    var_dump($row);die;
		    $sum1 += $qty    = $row['qty']>0 ? -abs($row['qty']) : abs($row['qty']);   //销售在数据库中是负数 在统计的时候应该是正数
			$sum3 += $amount = $row['amount'];                      //销售收入

            $qty = $row['qty']>0 ? -abs($row['qty']):abs($row['qty']);
            $unitCost = isset($info['inprice'][$row['invId']][$row['locationId']]) ? $info['inprice'][$row['invId']][$row['locationId']] : 0;   //单位成本
            $unitCost = round($unitCost,2);
            $sum8 += $unitCost;
            $cost1 = $unitCost * $qty;
            $cost = round($cost1,2);//销售成本
            $sum9 += $cost;
            $saleProfit = round($amount - $cost,2);//销售毛利=销售收入-销售成本
            $sum10 += $saleProfit;
            $salepPofitRate1 = $amount>0? ($saleProfit/$amount)*100 :0;
            $salepPofitRate = round($salepPofitRate1,2).'%';//毛利率
            $sum11 += $salepPofitRate;
			$v[$arr]['billId']        = intval($row['iid']);
		    $v[$arr]['billNo']        = $row['billNo'];
			$v[$arr]['billType']      = $row['billType']; 
			$v[$arr]['date']          = $row['billDate']; 
			...
			$v[$arr]['description']   = $row['description']; 
			$v[$arr]['skuId']         = 0; 
			$v[$arr]['cost']          = $cost;   //销售成本
			$v[$arr]['unitCost']      = $unitCost;   //单位成本
			$v[$arr]['saleProfit']    = $saleProfit;   //销售毛利
			$v[$arr]['salepPofitRate']= $salepPofitRate;   //销售毛利率
		...
		$data['data']['userdata']['cost']        = $sum9;
		$data['data']['userdata']['unitCost']    = $sum8;
		$data['data']['userdata']['saleProfit']      = $sum10;
		$data['data']['userdata']['salepPofitRate']  = $sum11;
		die(json_encode($data));
	}
	...
	$sum9 = $sum4>0? ($sum8/$sum4)*100 :0;
	    $sum11 = $sum4>0? ($sum10/$sum3)*100 :0;
	    $data['status'] = 200;
	    $data['msg']    = 'success';
	    $data['data']['page']      = $page;
...
//        array_unshift($colIndex,'invNo','invName','spec','unit','qty_1','cost_1');
		array_unshift($colNames,'商品编号','商品名称','规格型号','单位','数量','单位成本','成本');
		array_unshift($colIndex,'invNo','invName','spec','unit','qty_1','price_1','cost_1');
		...
		foreach ($list as $arr=>$row) {
			...
			foreach($storage as $arr1=>$row1) {
                ...
                $v[$arr]['price_'.$i] = round(($row['qty'.$i] * $price)/($row['qty'.$i]),3);//$row['price'];
            ...
            $v[$arr]['price_1'] = round($cost1/$row['qty'],3);//$row['price'];
