...
据，成功导入：0条数据，失败：0条数据。<br/>","status":2,"spendTime":0}],"totalsize":3}}');  
	...
	public function uploadExcel() {
		...
		$good['vipPrice'] = floatval($row[13]);
		$good['discountRate1'] = floatval($row[14]);
		$good['discountRate2'] = floatval($row[15]);
		$good['remark']= ($row[17]);
		$good['udf03']= ($row[18]);
		$good['udf04']= ($row[19]);
		$good['udf05']= ($row[20]);
                ...
                $info = array(
                    ...
                    'locationName','unitName','categoryName','pinYin','sonGoods','dopey','wholesalePrice','remark','vipPrice','discountRate1','discountRate2','udf03','udf04','udf05'
                );
...
//                 if($this->mysql_model->get_count('goods',array('isDelete'=>0,'number'=>$good['number'])) <= 0){
                ...
                if(!empty($row[22])){
                    $list = $this->mysql_model->get_rows('storage',array('isDelete'=>0,'name'=>$row[22]));
                    if (count($list) > 0) {
                        $arr = 0;
                        $v[$arr]['invId']         = $rtn['id'];
                        $v[$arr]['locationId']    = $list['id'];
                        $v[$arr]['qty']           = floatval($row[24]);
                        $v[$arr]['price']         = floatval($row[25]);
                        $v[$arr]['amount']        = floatval($row[24])*floatval($row[25]);
                        $v[$arr]['skuId']         = 0;
