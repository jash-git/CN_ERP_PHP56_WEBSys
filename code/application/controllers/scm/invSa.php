...
class InvSa extends CI_Controller {
    ...
    public function rsBatchCheckInvSa() {
	    $this->common_model->checkpurview(90);
	    $id   = str_enhtml($this->input->post('id',TRUE));
	    $this->mysql_model->get_count('verifica_info','iid in('.$id.')')>0 && str_alert(-1,'存在关联“收款单据”，无法删除！请先在“收款单”中删除该销货单！');//add 
	    //$this->mysql_model->get_count('verifica_info','(billId='.$id.')')>0 && str_alert(-1,'存在关联“收款单据”，无法删除！请先在“收款单”中删除该销货单！');//add 
	    //weiyouj  2018-04-30 屏蔽
    ...
    public function toPdf() {
		...
		if (count($data)>0) { 
			//$data['num']    = 8;
			//add begin  weiyouj 2018-04-28 添加设置打印行数
			$cexo_topdf=8;
			...
			try {
			    $html2pdf = new HTML2PDF('landscape', 'A5', 'en',true,'UTF-8',[0,0,0,0]);
				$html2pdf->setDefaultFont('javiergb');
				$html2pdf->pdf->SetDisplayMode('fullpage');//fullpage  fullwidth  real  default
	...
	private function validform($data) {
		...
		//add by michen 20171210 for 修改时库存验证
		if($system['requiredCheckStore']==1 && $data['id']>=1){
    		if (intval($data['transType'])==150601) {
    		    $lastdata = $this->data_model->get_invoice('a.id='.$data['id'].' and a.billType="SALE"',1);
		...
		foreach ($data['entries'] as $arr=>$row) {
			...
			if ($system['requiredCheckStore']==1 && $data['id']<1) {  
			    ...
			    //add by michen 20171210 for 修改时库存验证
		        if (isset($inventory[$row['invId']][$row['locationId']])) {
		            $inventory[$row['invId']][$row['locationId']] < ($row['qty']-$newData[$row['invId']][$row['locationId']]) && str_alert(-1,$row['locationName'].$row['invName'].'商品库存不足！');
		        }else {
				    $gooddata = $this->mysql_model->get_results('goods','(id ='.$row['invId'].') and (isDelete=0)');
				    if(count($gooddata)>0){
				        str_alert(-1,'商品'.$row['invName'].'库存不足！');
