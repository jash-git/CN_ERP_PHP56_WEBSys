...
class Data_model extends CI_Model{
	...
	public function get_invoice_info_inventory() {
		$list = $this->mysql_model->query('select invId,locationId,sum(qty) as qty from '.$this->db->dbprefix('invoice_info').' where isDelete=0 group by invId,locationId',2);//add by michen 20170917 for where isDelete=0
	...
	public function get_invBalance($where='',$select='',$type=2) {
	    $sql = 'select 
		            a.invId,a.locationId,a.price,sum(a.qty) as qty,sum(a.amount) as amount,
					'.$select.'
	    ...
	    $sql = 'select 
				...
				left join 
					(select 
						...
						round(sum(case when billType="INI" then amount else 0 end)/sum(case when billType="INI" then qty else 0 end),2) as iniunitCost,
